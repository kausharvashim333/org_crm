...
import { GraduationCap, Lock, Mail, ArrowLeft, Building2, KeyRound, X, CheckCircle2, AlertCircle, RefreshCw, Eye, EyeOff, ArrowRight, Loader2 } from 'lucide-react';
...
export default function PartnerLogin() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const el = document.getElementById('partner-email-input');
      if (el) el.focus();
    }, 300);
  }, [slug]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (rememberMe) {
        localStorage.setItem('partner_remember_email', email);
      } else {
        ...
        <div className="flex items-center gap-3 z-10">
          <div className="w-12 h-12 bg-white/10 border border-white/20 rounded-xl flex items-center justify-center backdrop-blur-md overflow-hidden">
            {orgLogo && typeof orgLogo === 'string' && orgLogo.trim() !== '' && orgLogo !== 'undefined' ? (
              <img src={orgLogo} alt="logo" className="w-full h-full object-cover rounded-xl" onError={(e) => { const img = e.target; if (!img.dataset.retried && orgLogo.includes('/uploads/')) { img.dataset.retried = 'true'; const path = orgLogo.substring(orgLogo.indexOf('/uploads/')); img.src = `/api${path}`; } else { img.style.display = 'none'; } }} />
            ) : (
      ...
      <div className="w-full md:w-1/2 lg:w-2/5 min-h-screen bg-slate-50 flex items-center justify-center p-8 relative">
        ...
        <div className="absolute top-8 left-8 md:hidden flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center overflow-hidden bg-white border border-slate-100 shadow-sm">
            {orgLogo && typeof orgLogo === 'string' && orgLogo.trim() !== '' && orgLogo !== 'undefined' ? <img src={orgLogo} alt="logo" className="w-full h-full object-cover" onError={(e) => { const img = e.target; if (!img.dataset.retried && orgLogo.includes('/uploads/')) { img.dataset.retried = 'true'; const path = orgLogo.substring(orgLogo.indexOf('/uploads/')); img.src = `/api${path}`; } else { img.style.display = 'none'; } }} /> : <GraduationCap className="w-4 h-4 text-slate-700" />}
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100/80 p-8 w-full max-w-md transition-all duration-300 hover:shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  required 
                  className="w-full px-4 py-3 pl-10 pr-10 border border-slate-200 rounded-xl focus:outline-none transition-all text-sm bg-slate-50 text-slate-850" 
                  placeholder="••••••••" 
            ...
            >
              {loading ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> : <>Verify & Log In <ArrowRight className="w-4 h-4" /></>}
            </button>
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none pt-1">
              ...
              />
              <span className="text-xs font-medium text-slate-500">Remember my email</span>
            </label>
          </form>
          ...
          <div className="mt-6 text-center pt-5 border-t border-slate-100 space-y-3">
            <div className="flex flex-col sm:flex-row gap-2 justify-center">
              ...
              <Link 
                to="/franchise/apply" 
                className="inline-flex items-center justify-center gap-1.5 px-5 py-2.5 text-xs font-bold rounded-xl transition-all hover:scale-[1.02] text-white"
                style={{ backgroundColor: themeColor }}
              >
                <Building2 className="w-3.5 h-3.5" /> Apply for Partnership
              </Link>
            </div>
          </div>
        </div>
