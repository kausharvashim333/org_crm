...
export default function AdminCourses() {
  ...
  const handleCTSubmit = async (e) => {
    ...
    try {
      await createCenterType({ name: ctForm.name.trim() });
      showSuccess('Center type added');
      setCtForm({ name: '' });
      loadCenterTypes();
    } catch { showError('Failed to add center type'); }
  };

  const handleCTRename = async (id) => {
    if (!renameCTName.trim()) return;
    try {
      await renameCenterType(id, renameCTName.trim());
      showSuccess('Center type renamed — all courses updated');
      setEditCT(null);
      setRenameCTName('');
      loadCenterTypes();
      load();
    } catch (err) {
      showError(err.response?.data?.message || 'Failed to rename');
    }
  };

  const handleCTDelete = async (id) => {
    if (!confirm('Delete this center type? Make sure no courses are using it.')) return;
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setShowCTModal(true); setCtForm({ name: '' }); setEditCT(null); }} className="btn-secondary flex items-center gap-2 text-xs py-2 whitespace-nowrap">
      ...
      <Modal isOpen={showCTModal} onClose={() => { setShowCTModal(false); setEditCT(null); setRenameCTName(''); }} title="Manage Center Types" size="md">
        <form onSubmit={handleCTSubmit} className="space-y-3">
          <label className="block text-sm font-medium mb-1">Add New Center Type</label>
          <div className="flex gap-2">
            <input type="text" required value={ctForm.name} onChange={(e) => setCtForm({ name: e.target.value })} className="input-field flex-1" placeholder="e.g. Beauty & Wellness, Agriculture" />
            <button type="submit" className="btn-primary px-4 py-2 text-sm font-bold whitespace-nowrap">Add</button>
          </div>
        </form>
        ...
        <div className="mt-5 pt-4 border-t">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {centerTypes.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-4">No center types yet.</p>
            ) : centerTypes.map(ct => (
              <div key={ct._id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                {editCT && editCT._id === ct._id ? (
                  <div className="flex items-center gap-2 flex-1">
                    <input type="text" value={renameCTName} onChange={(e) => setRenameCTName(e.target.value)} className="input-field flex-1 text-sm" placeholder="New name" autoFocus onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleCTRename(ct._id); } }} />
                    <button onClick={() => handleCTRename(ct._id)} className="btn-primary px-3 py-1.5 text-xs font-bold">Save</button>
                    <button onClick={() => { setEditCT(null); setRenameCTName(''); }} className="btn-secondary px-3 py-1.5 text-xs font-bold">Cancel</button>
                  </div>
                ) : (
                  <>
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-indigo-100 text-indigo-600 font-bold text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => { setEditCT(ct); setRenameCTName(ct.name); }} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg border border-indigo-100" title="Rename">
                        <Edit className="w-3.5 h-3.5" />
                      </button>
          ...
          <p className="text-[10px] text-slate-500 mt-3">Renaming a center type automatically updates all courses using it. Deletion is blocked if courses are using it.</p>
        </div>
      </Modal>
    </div>
  );
